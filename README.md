...
If your organization has a compliance obligation â€” GDPR, CIPA, FERPA, HIPAA, SOC 2, or an internal acceptable-use policy â€” "we block the AI tools we happen to know about" is not a defensible control. This database is the control.
...
**Free sample:** a 500-row CSV is available with no signup â€” https://www.aitoolsblocklist.com/sample_databases/ai_tools_database_sample.csv
...
- **CSV / JSON** â€” the full classified dataset, for pipelines, SIEMs, and analytics.
- **REST API** â€” real-time single-domain lookup, filtered list retrieval, bulk classification, and a delta feed.
- **EDL (External Dynamic List)** â€” a plain newline-delimited domain list that next-generation firewalls poll on a schedule.
- **PAC file** â€” a `FindProxyForURL` function that routes matched domains to a block proxy.
- **Hosts file** â€” `0.0.0.0 domain` lines for endpoint-level and Pi-hole-style enforcement.
- **DNS RPZ** â€” a Response Policy Zone for resolver-level blocking (BIND, Unbound, Knot).
...
A domain that is not an AI tool returns `"in_blocklist": false` with `200 OK` â€” the absence of a match is an answer, not an error.
...
### Delta feed â€” stay current without re-downloading everything
...
The `/v1/domains` endpoint accepts query parameters that compose: `category`, `subcategory`, `min_confidence`, `risk_tier`, and `updated_since`. Results are cursor-paginated â€” each response carries a `next_cursor` you pass back to retrieve the following page, which keeps pagination stable even as the underlying dataset changes between requests.
...
Configure the firewall's external-dynamic-list object to poll that URL hourly, then attach it to a security policy with an action of `deny`. No commits are needed for content updates â€” the firewall re-reads the list on its interval.
...
1. **Ingestion.** Newly-registered and newly-observed domains enter from certificate-transparency logs, zone-file deltas, and the crawl corpus. This is the funnel â€” hundreds of thousands of candidates per day, the vast majority of which are not AI tools.
...
Because the whole pipeline is deterministic given its inputs, two lookups of the same domain on the same day return identical classifications â€” important when you are diffing enforcement state across sites.
...
- IETF DNS Response Policy Zones â€” https://datatracker.ietf.org/doc/html/draft-vixie-dns-rpz
- IANA root zone and TLD registry â€” https://www.iana.org/domains/root
- Mozilla PAC / `FindProxyForURL` specification â€” https://developer.mozilla.org/en-US/docs/Web/HTTP/Proxy_servers_and_tunneling/Proxy_Auto-Configuration_PAC_file
- ICANN registration data (WHOIS/RDAP) â€” https://www.icann.org/resources/pages/rdap-2019-08-05-en
- NIST guidance on protecting controlled unclassified information â€” https://csrc.nist.gov/publications/detail/sp/800-171/rev-2/final
...
Maintained by the team behind https://www.aitoolsblocklist.com â€” questions and integration help: `info@alpha-quantum.com`.
